...
import { Box, VStack, FormControl, FormLabel, RadioGroup, Radio, Button, Flex, Text } from '@chakra-ui/react';

function calculateQuizScore(answers, quizBank) {
    let totalScore = 0;
...
quizBank.forEach((item) => {
  // Check if the issue is "ADHD"
  console.log(`item.issue: '${item.issue}'`)
  if (item.issue.trim() === "ADHD") {
    // Assign the questions to the adhdQuestions variable
    questionz = item.questions;
  }
});
...
const FormPage = () => {
  // State to track answers and pagination
  const [answers, setAnswers] = useState({});
  const [currentPage, setCurrentPage] = useState(1);
  const questionsPerPage = 3;

  // Find ADHD questions
  const adhdQuestions = quizBank.find(item => item.issue === "ADHD")?.questions || [];

  // Calculate the start and end index for questions to display on the current page
  const startIndex = (currentPage - 1) * questionsPerPage;
  const endIndex = startIndex + questionsPerPage;
  const displayedQuestions = adhdQuestions.slice(startIndex, endIndex);
  ...
  const handleSubmit = (e) => {
      e.preventDefault();
      console.log(calculateQuizScore(answers, quizBank))
      console.log(answers);
  };

  // Handle pagination
  const handleNextPage = () => {
      if (currentPage * questionsPerPage < adhdQuestions.length) {
          ...
          <Box p={4} w="400px" borderWidth="1px" borderRadius="lg">
              <VStack spacing={4} align="stretch" as="form" onSubmit={handleSubmit}>
                  ...
                  <Flex justify="space-between" mt={4}>
                      ...
                      <Button onClick={handleNextPage} disabled={endIndex >= adhdQuestions.length}>
                          Next
                      </Button>
                  </Flex>
